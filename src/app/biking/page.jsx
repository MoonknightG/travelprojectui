...
export default function BikingPage() {
  const bikingTrips = trips.filter(
    (trip) => trip.category === "Biking"
  );
  ...
  return (
    <section className="relative min-h-screen bg-gradient-to-br from-[#0B0F1A] via-[#0F172A] to-black text-white py-24">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-24 right-0 w-96 h-96 bg-amber-500/10 rounded-full blur-3xl" />
        <div className="absolute bottom-20 left-0 w-80 h-80 bg-yellow-600/10 rounded-full blur-3xl" />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 items-center mb-14">
          ...
          <div>
            ...
            <p className="text-slate-300 max-w-md leading-relaxed">
              Premium biking adventures across mountains, valleys and
              legendary highways — crafted for thrill, comfort
        ...
        <div>
          <p className="text-xs uppercase font-semibold text-amber-400 mb-8">
            Explore Biking Trips
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {bikingTrips.map((trip) => (
              <Link
                key={trip.slug}
              ...
              >
                {/* IMAGE */}
                <div className="relative h-56 overflow-hidden hover:shadow-amber-900/40 transition">
                  <Image
                    src={trip.image}
                    alt={trip.title}
                    fill
          ...
          {bikingTrips.length === 0 && (
            <p className="text-slate-400 mt-10">
              No biking trips available right now.
            </p>
          )}
        </div>
